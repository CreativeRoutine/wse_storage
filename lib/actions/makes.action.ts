  ...
  try {
    ...
    const makes = await Makes.find({ _id: _id })
      .populate({
        path: "printers",
        model: Printer,
        select: "_id sn barcode createdOn name",
      })
      .lean();
...
export async function deleteMake(params: DeleteMakeParams) {
  try {
    ...
    if (!make) {
      return {
        success: false,
        message: "Make can't be deleted!",
        info: "This Make does not exist in the database",
      };
    }
    if (make.printers.length != 0) {
      return {
        success: false,
        message: "Make can't be deleted!",
        info: "This supplier has pallets or printers. Please delete them first",
      };
    }
  ...
  try {
    ...
    if (printers.length === 0) {
      return {
        ...
        info: `No printers found for product number: ${make.productNumber}`,
      };
    }
    ...
    return {
      success: true,
      message: `Make preview updated successfully! ${updateResult.modifiedCount} printers updated.`,
    };
  } catch (error) {
    console.error("Error updating Make and Printers:", error);
    return {
      success: false,
      message: "An error occurred while updating the Make and Printers",
    };
  }
}
