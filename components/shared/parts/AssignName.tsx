...
export default function AssignName({ id }: Props) {
  ...
  async function onSubmit(values: z.infer<typeof assignNameSchema>) {
    ...
    try {
      ...
      router.push(`/settings/printer-parts/${values.printerName}`);
      // router.refresh();
      ...
      <div className="mb-4">
        ...
        <Form {...form}>
          ...
          >
                  ...
                  render={({ field }) => (
                    ...
                    <FormItem>
                      ...
                      <FormDescription className="text-red-400">
                        Use this form if no printers this model were added.{" "}
                      </FormDescription>
                      <FormControl>
                        <div className="flex">
                          <Input
            ...
            >
              {isSubmitting ? (
                <>{type === "edit" ? "Editing ..." : "Renamimg ..."}</>
              ) : (
                <>{type === "edit" ? "Edit pallet" : "Rename parts section"}</>
              )}
            </Button>
          </form>
        </Form>
