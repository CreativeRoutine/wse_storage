...
import { addPart, createPrinterPart, takeOffPart } from '@/lib/actions/parts.action';
...
export default function TakeOffPart ({ printerID, productNumber }: Props){
  ...
  async function onSubmit(values: z.infer<typeof addPartSchema>) {
    ...
    try {
      ...
      const response:any = await takeOffPart({
        _id: printerID,
        printerProductNumber: values.printerProductNumber,
