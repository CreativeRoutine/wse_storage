...
export default function AddPart({ parts }: any) {
  ...
  const handleProductNumberChange = (value: string) => {
    ...
    const selectedPart = parts.find((part: any) => part.printerName === value);
  ...
  async function onSubmit(values: z.infer<typeof addGenericPartSchema>) {
    ...
    try {
      //Make test before it goes to the server
      // need to find out if this barcode ALREADY USED IN THE SYSTEM or not
      // 
              ...
              <FormItem>
                ...
                <FormControl>
                  ...
                  >
                    ...
                    >
                      <SelectValue placeholder="Select Printer by name" />
                    </SelectTrigger>
                    
                    <SelectContent className="bg-dark-400 p-0 text-white border-0">
                      <SelectGroup className="py-4">
                        {parts.map((part: any) => (
                          <SelectItem key={part.printerName} value={part.printerName} className="py-2 text-white hover:bg-dark-200">
                            {part.printerName}
                          </SelectItem>
                        ))}
                      </SelectGroup>
                      ...
                      <SelectContent className="bg-dark-400 p-0 text-white border-0">
                        <SelectGroup className="py-4">
                          {partsNames.map((name: string, index: number) => (
                            <SelectItem key={index} value={name} className="py-2 text-white hover:bg-dark-200 flex flex-row justify-between">
                              {name}
                            </SelectItem>
                          ))}
                        </SelectGroup>
