...
import {
  addPartModelToCollection,
  deletePartModelFromCollection,
...
export function PartsSwitcher({ label, printerPN, state }: Props) {
  ...
  async function settingsChanged(checked: boolean) {
    try {
      if (checked) {
        // console.log("DATA===>", printerPN, label, checked);
        const response = await addPartModelToCollection(
          printerPN,
          label,
          checked
        );
      ...
      toast({
        title: "An error occurred while changing the setting",
        variant: "destructive",
      });
    }
  }
