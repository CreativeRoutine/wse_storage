...
import { deleteWork } from '@/lib/actions/work.action';
import { usePathname} from 'next/navigation';

const DisplayWorks = ({parts}:any) => {

  
  const usepathname = usePathname();

  const handleDelete = async (workId: any) => {
    try {
      await deleteWork({workId, path: usepathname,}); // Call the deletePrinter function with the printerId
  ...
  return (
    <div className="bg-secondary-200 px-8 mb-6 py-6 w-full rounded-xl border border-dark-350 shadow-lg">
      <div className="mb-4">
        <div className="mb-3 pl-4 text-lg text-slate-300 font-semibold">List of recent works:</div>
        <div className="">
          <ul className="w-full">
          <li className="group flex flex-row justify-between items-center rounded-xl bg-dark-200 text-white font-semibold p-4">
            <span className='w-1/2 flex justify-start'>Name</span>
            ...
            {parts.map((part: any) => (
              ...
              <li key={part._id} className="group flex flex-row justify-between items-center rounded-xl hover:bg-dark-200 p-2">
                <span className="w-1/2 pl-2 text-slate-500 group-hover:text-slate-300">{part.name}</span>
                {/* <span className='text-slate-500 group-hover:text-slate-300'>{part.pn}</span> */}
                <Button type="submit" className="p-2 text-slate-600 group-hover:bg-red-500 group-hover:text-white p-2 rounded-xl" onClick={() => handleDelete(work._id)}>
                  Delete work
                </Button>
              </li>
            ))}
