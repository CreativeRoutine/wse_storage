  ...
  const handleFormSubmit = async () => {
    ...
    const createFormData = () => {
      ...
      return {
        ...
        timeSpent: savedTime,
        status:
          afterRefurbish === "Disassembled" ? "Disassembled" : "Refurbished",
    ...
    try {
      ...
      if (!printerResponse.success) {
        toast({
          title: "Error",
          description: printerResponse.message,
          variant: "custom",
        });
        return;
      }
    ...
    <div className="bg-secondary-200 px-6 mb-1 py-6 w-full rounded-xl border border-dark-350 shadow-lg">
          ...
          {user && user.name != "User not selected!" ? (
            <div>
              <div className="text-slate-400 text-base font-normal">
                Tech's name:{" "}
                <span className="text-white font-bold ml-1">{user.name}</span>
              </div>
            ...
            <div className="my-2 flex gap-4 flex-row justify-between">
              <div className="font-bold text-normal text-white mb-2 flex flex-col ">
                ...
                <ul className="space-y-1 text-slate-400 font-normal text-sm">
                  <li>
                    Make:
                    <span className="text-white ml-2">{printerData.name}</span>
                  </li>
                  <li>
                    Barcode:
                    ...
                    return (
                      <>
                        {lastTask ? (
                          <>
                            <li>
                              {switchState ? "Дата:" : "Date:"}{" "}
                              <span className="text-white ml-2">
                                {new Date(lastTask.date).toLocaleString()}
                              </span>
                            </li>
                            <li>
                              {switchState ? "Имя тека" : "Performed By:"}{" "}
                              <span className="text-white font-bold ml-2">
                                {lastTask.user?.name || "Unknown"}
                              </span>
                            </li>
                            <li>
                              {switchState
              ...
              <ul className="">
                ...
                <li className="font-normal">
                  Cleanliness:{" "}
                  <span className="text-green-500 font-md">{cleanliness}</span>
                </li>
                <li className="font-normal">
                  Workable:{" "}
                  <span className="text-green-500 font-md">{workable}</span>
                </li>
                <li className="font-normal">
                  Parts changed:{" "}
                  <span className="text-green-500 font-md flex flex-wrap">
                    {changedParts.map((part) => `${part}, `)}
                  </span>
                </li>
                <li className="font-normal">
                  Tested:{" "}
                  <span className="text-green-500 font-md">
                    {testedFunctions.map((part) => `${part}, `)}
                  </span>
                </li>
                <li className="font-normal">
                  Total page count:{" "}
                  <span className="text-green-500 font-md">{pagesNumber}</span>
                </li>
                <li className="font-normal">
                  After refarbish:{" "}
                  <span className="text-green-500 font-md">
      ...
      <div className="py-2 mb-2">
        <FindPrinter
          label={
            switchState
              ? "Введите баркод принтера:"
              : "Search for a printer by barcode:"
          }
          printer={setPrinterData}
          reset={resetForms}
        />
      </div>
