...
import {
  deletePrinterPartFromStorage,
  getListOfAllPrinters,
  getPartsByName,
  getPartsByNameFromTechForm,
...
export default function PartsChanged({
  ...
  async function getPrinterData() {
    try {
      ...
      console.log("printers list received onload", printers);
  ...
  async function findParts() {
    try {
      console.log("THIS IS printerName", printerName);
      const response: any = await getPartsByNameFromTechForm({
        currentPrinter: printerName,
      });

      console.log("List of parts loaded upon printer selected", response);
  ...
  const handlePartChange = (value: any) => {
    ...
    console.log("Part name selected", value);
  };
  ...
  async function onSubmit(values: z.infer<typeof searchForPartSchema>) {
    ...
    try {
      ...
      toast({
        ...
        description:
          "An error occurred while replacing the part. Please try again.",
        variant: "destructive",
      });
    }
  }
              ...
              <>
                <FormField
                  ...
                  render={({ field }) => (
                    // First Input
                    <FormItem>
                      <FormLabel className="mb-3 text-base text-slate-300 font-semibold">
                        Barcode:
                      </FormLabel>
                      <FormControl>
                        <div className="flex">
