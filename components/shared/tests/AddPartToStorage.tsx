...
import { z } from "zod";
import { addPartToStorageSchema } from "@/lib/validations";
...
import { addPrinterPartToStorage } from '@/lib/actions/partsTest.action';


const type:any = 'create';

export default function AddPartToStorage (){
  ...
  // 1. Define your form.
  // addPartToStorageSchema took from lib/validations.ts to validate the form
  const form = useForm<z.infer<typeof addPartToStorageSchema>>({
    resolver: zodResolver(addPartToStorageSchema),
    defaultValues: {
      productNumber:"",
      barcode:"",
      location:"",
  ...
  // addPartToStorageSchema took from lib/validations.ts to validate the form
  async function onSubmit(values: z.infer<typeof addPartToStorageSchema>) {
    ...
    try {
      ...
      const response = await addPrinterPartToStorage({
              ...
              <div className="w-full">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    // First Input
                    <FormItem>
                      <FormLabel className="mb-3 text-base text-slate-300 font-semibold">Location:</FormLabel>
                      <FormControl>
                        <div className="flex">
                          <Input
                            className="w-full mb-4 ouline-none bg-dark-600 text-white border-0 rounded-lg no-focus"
